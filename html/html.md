...
### Спецсимволы
Это особые строки, которые начинаются с амперсанда (`&`) и заканчиваются точкой с запятой (`;`). Например, знак меньше на страницу можно вставить спецсимволом `&lt;` (less than), а знак больше мнемоникой `&gt;` (greater than):
